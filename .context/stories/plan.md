...
| 038 | Make daily planning idempotent | not-started | 003, 013, 037 |
| 039 | Add a read-only weekly schedule preview | not-started | 003, 009, 037, 038 |
| 040 | Prevent Telegram polling conflicts from operator jobs | not-started | 004, 011, 014, 033 |
| 041 | Align migration tests with production bootstrap | not-started | 002, 014, 037 |
...
- 2026-08-03: Added Stories 038–041 after end-to-end verification exposed scheduler idempotency, weekly preview, Telegram single-poller, and migration-test hardening gaps. Story 038 fixes repeated production day planning so it does not persist duplicate open occurrences. Story 039 adds a true read-only, state-carrying weekly preview instead of using repeated `PlanDay` calls for inspection. Story 040 prevents CLI operator jobs from starting a second Telegram long-poller. Story 041 aligns migration tests with the real bootstrap chain so later migrations are validated on a clean database.
