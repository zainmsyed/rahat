...
- Calendar endpoints: status, auth-url, connect (verifies the OAuth state's user matches the session), and disconnect via new `calendar.Service.DisconnectGoogle` / `GoogleConnectionStatus` (clears stored credentials; no `internal/store` change required, keeping the story in scope).
