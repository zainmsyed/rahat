...
describe('task management api', () => {
	...
	it('surfaces authentication and validation failures', async () => {
		...
		await expect(createTask({ name: '', description: '', duration_minutes: 10, cadence_type: 'interval', cadence_value: 1, priority: 'medium', time_of_day_preference: 'morning', day_preference: 'any', subtasks: [] })).rejects.toThrow('task name is required');
	});

	it('keeps internal subtask metadata when mapping an existing task to a draft', () => {
		const draft = toDraft({ id: 'task-1', name: 'Laundry', description: '', duration_minutes: 30, cadence_type: 'interval', cadence_value: 2, priority: 'medium', time_of_day_preference: 'morning', day_preference: 'any', is_multistep: true, is_paused: false, subtasks: [{ id: 'sub-1', name: 'Wash', duration_minutes: 10, time_of_day_preference: 'morning', dependency_type: 'soft_followup', min_gap_after_previous_minutes: 45 }] });
