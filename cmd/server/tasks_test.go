...
func TestTaskEditPreservesCompletedOccurrenceAndEventHistory(t *testing.T) {
	...
	body, _ = json.Marshal(payload)
	req = httptest.NewRequest(http.MethodPut, "/tasks/"+created.ID, bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Origin", "http://localhost:5200")
	req.AddCookie(cookie)
	rec = httptest.NewRecorder()
...
func TestTaskMutationsRejectUntrustedOrigins(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "create missing origin", method: http.MethodPost, path: "/tasks", body: `{}`},
		{name: "create wrong origin", method: http.MethodPost, path: "/tasks", body: `{}`, origin: "http://attacker.localhost:5200"},
		{name: "update wrong origin", method: http.MethodPut, path: "/tasks/task-id", body: `{}`, origin: "http://attacker.localhost:5200"},
		{name: "pause wrong origin", method: http.MethodPost, path: "/tasks/task-id/pause", body: `{"paused":true}`, origin: "http://attacker.localhost:5200"},
		{name: "remove wrong origin", method: http.MethodDelete, path: "/tasks/task-id", origin: "http://attacker.localhost:5200"},
	}
