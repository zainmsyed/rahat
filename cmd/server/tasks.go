...
func (h *taskManagementHandler) handleUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if existing.Task.ArchivedAt != nil {
		http.Error(w, "removed tasks cannot be edited", http.StatusBadRequest)
		return
	}
	var req onboardingTaskRequest
	if err := decodeJSON(r, &req); err != nil {
...
func (h *taskManagementHandler) handlePause(w http.ResponseWriter, r *http.Request) {
	...
	if existing.Task.ArchivedAt != nil {
		http.Error(w, "removed tasks cannot be paused or resumed", http.StatusBadRequest)
		return
	}
	...
	if _, err := h.tasks.PauseTask(r.Context(), taskID, req.Paused); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
...
func (h *taskManagementHandler) handleArchive(w http.ResponseWriter, r *http.Request) {
	...
	taskID := strings.TrimSpace(r.PathValue("taskID"))
	if _, err := h.requireOwnedTask(r, current.User.ID, taskID); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	if err := h.tasks.ArchiveTask(r.Context(), taskID); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
...
func (h *taskManagementHandler) requireTrustedOrigin(w http.ResponseWriter, r *http.Request) bool {
	if err := h.auth.requireTrustedOrigin(r); err != nil {
...
func (h *taskManagementHandler) requireOwnedTask(r *http.Request, userID, taskID string) (taskpkg.TaskWithSubtasks, error) {
	if taskID == "" {
		return taskpkg.TaskWithSubtasks{}, errors.New("missing task id")
	}
	taskDef, err := h.tasks.GetTaskWithSubtasks(r.Context(), taskID)
	if err != nil {
		return taskpkg.TaskWithSubtasks{}, err
	}
	if taskDef.Task.UserID != userID {
		return taskpkg.TaskWithSubtasks{}, errors.New("task does not belong to this user")
	}
	return taskDef, nil
}
